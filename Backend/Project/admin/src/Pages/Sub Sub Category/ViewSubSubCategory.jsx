...
export default function ViewCategory() {
  ...
  useEffect(() => {
    axios.post(`${ import.meta.env.VITE_BASE_URL }/${ import.meta.env.VITE_SUB_SUB_CATEGORY_API }/view`, {
      page: currentPage,
      name: searchName,
  ...
  const allCheckBoxSelect = () => {
    if (checkBoxValues.length == subSubCategories.length) {
      setCheckBoxValues([]);
      setButtonDisabled(true);
    } else {
      setCheckBoxValues([]);
  ...
  const changeStatus = () => {
    axios.put(`${ import.meta.env.VITE_BASE_URL }/${ import.meta.env.VITE_SUB_SUB_CATEGORY_API }/change-status`, {
      ids : checkBoxValues
    })
      .then((result) => {
        if (result.data._status == true) {
                      ...
                      <th scope="col" class="p-4">
                        <div class="flex items-center">
                          <input id="checkbox-all-search"
                            ...
                            checked={checkBoxValues.length == subSubCategories.length ? 'checked' : ''}
