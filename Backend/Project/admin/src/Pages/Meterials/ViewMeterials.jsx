...
export default function ViewCategory() {
  ...
  useEffect(() => {
    axios.post(`${ import.meta.env.VITE_BASE_URL }/${ import.meta.env.VITE_MATERIAL_API }/view`, {
      page: currentPage,
      name: searchName
    })
      .then((result) => {
  ...
  const allCheckBoxSelect = () => {
    if (checkBoxValues.length == materials.length) {
      setCheckBoxValues([]);
      setButtonDisabled(true);
    } else {
      setCheckBoxValues([]);
  ...
  const changeStatus = () => {
    axios.put(`${ import.meta.env.VITE_BASE_URL }/${ import.meta.env.VITE_MATERIAL_API }/change-status`, {
      ids : checkBoxValues
    })
      .then((result) => {
        if (result.data._status == true) {
  ...
  const deleteRecords = () => {
    axios.put(`${ import.meta.env.VITE_BASE_URL }/${ import.meta.env.VITE_MATERIAL_API }/delete`, {
      ids : checkBoxValues
    })
      .then((result) => {
        if (result.data._status == true) {
  ...
  return (
    <section className="w-full">
      ...
      <Breadcrumb path={"Material"} link={"/materials/view-material"} path2={"View"} slash={"/"} />
                      ...
                      <th scope="col" class="p-4">
                        <div class="flex items-center">
                          <input id="checkbox-all-search"
                            ...
                            checked={checkBoxValues.length == materials.length ? 'checked' : ''}
